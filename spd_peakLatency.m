% spd_plot.m gets spd data depending on selected time_bins / alignments then
% visualize them
...
if ismac
    ...
    result_path = '/Research/smg_decisionPrior/results/spd/';
...
align_mode = {'preCue_cf','lateMem_cf','go'};%,'touch'
...
if pool_monkey, sel_monkey = {'allMonkeys'}; end
for mm = 1:numel(sel_monkey)
    cur_m =  ~(logical(cellfun(@isempty, strfind(new_filename,sel_monkey{mm}))));
    for ar = 1:numel(sel_area)
        cur_ar = ~(logical(cellfun(@isempty, strfind(new_filename,sel_area{ar}))));
        % combine all alignments
        spd_all = [];
        settings = cell(numel(align_mode),1);
        aligns = cell(numel(align_mode),1);
        timeBins = cell(numel(align_mode),1);
        nTimeBins = zeros(numel(align_mode)+1,1);
        
        for am = 1:numel(align_mode)
            cur_al = ~(logical(cellfun(@isempty, strfind(new_filename,align_mode{am}))));
            
            cur_file = find(cur_m & cur_ar & cur_al);
            
            disp(['Loading spd data from: ',mat_folder new_filename{cur_file} '.mat']);
            load([mat_folder new_filename{cur_file} '.mat'])
            settings{am} = setting;
            aligns{am} = align;
            timeBins{am} = time_bin;
            nTimeBins(am) = size(spd_table.spikeDensity,2);
            if ~isempty(spd_all)
                spd_table.(['spikeDensity_' align_mode{am}]) = spd_table.spikeDensity;
                spd_table.spikeDensity = [];
                spd_all = innerjoin(spd_all,spd_table,'Keys',{'unitName','fileIndex','trialIndex'},'RightVariables',['spikeDensity_' align_mode{am}]);
                spd_all.spikeDensity = [spd_all.spikeDensity,spd_all.(['spikeDensity_' align_mode{am}])];
                spd_all.(['spikeDensity_' align_mode{am}]) = [];
            else
                spd_all = spd_table;
            end
        end
        nTimeBins(am+1) = size(spd_all.spikeDensity,2);
        ...
        spd_all.unitName = categorical(spd_all.unitName);
        spd_all.condName = categorical(spd_all.condName);
        
        % take only full-bias cond
        sub_spd = spd_all(spd_all.BiasTypeAbs == 3 & spd_all.CuingType == 1,:);
        
        % compute maximal activity for each neuron when reaching to maximal direction
        %  & search for the time point when the maximal activity is reached
        spd_mean = grpstats(sub_spd,{'unitName','condName','moveDir'},'mean','DataVars','spikeDensity');
        spd_mean = unique(outerjoin(spd_mean,sub_spd,'Keys',{'unitName','condName','moveDir'},'Type','left','RightVariables',{'prefDir','maxDir'}),'rows');
        
        unit_maxAct = spd_mean(spd_mean.maxDir == spd_mean.moveDir,{'unitName','mean_spikeDensity','prefDir','maxDir'});
        unit_list = unit_maxAct.unitName;
        [unit_maxAct.maxAct,unit_maxAct.maxTime] = max(unit_maxAct.mean_spikeDensity,[],2);
...
%         spd_mean = unique(outerjoin(spd_mean,sub_spd,'Keys',{'unitName','condName','cueDir'},'Type','left','RightVariables',{'prefDir','maxDir','moveDir'}),'rows');
        ...
        tmp_dirDiff = abs(spd_mean.maxDir - spd_mean.prefDir);
        tmp_dirDiff(tmp_dirDiff>90) = 360 - tmp_dirDiff(tmp_dirDiff>90);
        spd_mean.card = tmp_dirDiff <= 22.5;
        
        % plot
        dir_list = unique(spd_all.(setting.dtun));
        n_dir = length(dir_list);
        
        fig_name = [new_filename{cur_file}(1:29) sel_area{ar} '_peakLatency'];
...
%     spd_mean = unique(outerjoin(spd_mean,spd_table,'Keys',{'unitName',setting.dtun,'condName'},'Type','left','RightVariables',{'prefDir','maxAct','maxDir'}),'rows');
%     if interpolated
...
%                                 'VariableNames',{'unitName','condName',[setting.dtun '_relat'],fr_name});
...
%                 unit_act = spd_mean(:,{'unitName','condName',fr_name});
